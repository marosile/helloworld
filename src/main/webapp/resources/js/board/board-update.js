...
$('#inputTag').keydown(function(event) {
    ...
    if (event.keyCode === 13) {
      ...
      $(this).val(null);
  } // else if (event.keyCode == 8 && !$(this).val().length) $('#tagsContainer #tagsDiv > span:last-of-type').remove();
});
...
$(document).on('click', 'span', function () {
    ...
    deleteTagAndSendAjaxRequest(trimmedTagName);
});

// keycode 8로 삭제
$(document).on('keydown', function (event) {
    if (event.keyCode === 8 && !$('#inputTag').val().length) { 
      const lastTag = $('#tagsContainer #tagsDiv span:last-of-type');
      if (lastTag.length) {
        const tagText = lastTag.text(); // 스팬의 텍스트 얻기
        const tagName = tagText.replace(/#/g, '').replace(/×/g, '').trim();
        deleteTagAndSendAjaxRequest(tagName);
        lastTag.remove();
      }
    }
});

  function deleteTagAndSendAjaxRequest(tagName) {
  
    $.ajax({
      type: 'POST', 
      url: '/board2/tagDelete',
